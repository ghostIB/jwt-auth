...
Реалізація системи автентифікації на основі JWT (JSON Web Tokens) 
та middleware в Go. Проєкт демонструє:
- Безпечне зберігання паролів через хешування (bcrypt)
- Генерацію та валідацію JWT токенів
- Захист API ендпоінтів через middleware
- Роботу з PostgreSQL через GORM ORM
